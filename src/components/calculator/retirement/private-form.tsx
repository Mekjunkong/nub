  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input label="Monthly Salary (THB)" type="number" value={values.monthlySalary ?? ""} onChange={(e) => onChange("monthlySalary", Number(e.target.value))} />
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input label="Monthly Contribution (THB)" type="number" value={values.monthlyContribution ?? ""} onChange={(e) => onChange("monthlyContribution", Number(e.target.value))} />
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <Input label="PVD Rate (%)" type="number" value={(values.pvdContributionRate ?? 0.05) * 100} onChange={(e) => onChange("pvdContributionRate", Number(e.target.value) / 100)} />
        <Input label="Employer Match (%)" type="number" value={(values.employerMatchRate ?? 0.05) * 100} onChange={(e) => onChange("employerMatchRate", Number(e.target.value) / 100)} />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input label="Expected Return (%)" type="number" value={(values.expectedReturn ?? 0.07) * 100} onChange={(e) => onChange("expectedReturn", Number(e.target.value) / 100)} />
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input label="Salary Growth Rate (%)" type="number" value={(values.salaryGrowthRate ?? 0.04) * 100} onChange={(e) => onChange("salaryGrowthRate", Number(e.target.value) / 100)} />
      </div>
    </div>
  );
}
